...
export default function Page() {
  ...
  const paymentState = sim2AllTrue && (faultCount1 > 0 || running) ? 'PAGO EFECTUADO' : 'PAGO NO EFECTUADO';
        ...
        <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 24 }}>
          <div style={box}>
            ...
            <div style={{ display: 'grid', gap: 8, marginTop: 16 }}>
              ...
              <div style={mini}>Fallo si corriente &gt; 40A o temperatura &gt;= 100°C.</div>
            </div>
          </div>
...
function ReadingRow({ reading }: { reading: Reading }) { const fault = parseFloat(reading.current) > 40 || parseFloat(reading.temperature) >= 100; return <div style={{ ...mini, borderColor: fault ? '#ef4444' : reading.status === 'spike' ? '#f59e0b' : '#1e293b' }}><b>t={reading.t}</b> · corriente {reading.current}A · temp {reading.temperature}°C {fault ? '· FALLA' : reading.status === 'spike' ? '· SPIKE' : ''}</div>; }
